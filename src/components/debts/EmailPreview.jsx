...
import { Copy, Download, CheckCircle2, Send, Lock, FileText, Info, ChevronLeft, Edit, Eye } from "lucide-react";
import { useToast } from "@/components/ui/use-toast";
import { Textarea } from "@/components/ui/textarea";

/**
 * EmailPreview - A beautiful email preview component that replaces plain textareas.
 * Shows the letter content as a formatted email with actions sidebar.
 *
 * Props:
 * - letterContent: string - The letter text content
 * - onLetterChange: (content) => void - Handler for editing the letter
 * - recipientName: string - Name of the recipient (creditor)
 * - recipientType: string - Type of recipient (e.g., "Schuldeiser", "Incassobureau")
 * - subject: string - Email subject line
 * - senderName: string - Name of the sender (user)
 * - onSend: () => void - Handler for marking as sent
 * - sendButtonText: string - Text for the send button
 * - onBack: () => void - Handler for going back
 * - backText: string - Text for back button
 * - calculation: object - VTLB calculation data for live display
 * - attachmentName: string - Optional attachment filename
 * - tipText: string - Optional tip text
 * - infoTitle: string - Optional info card title
 * - infoText: string - Optional info card text
 * - showDisclaimer: boolean - Show Juridisch Loket disclaimer
 * - showChecklist: boolean - Show checklist section
 */
export default function EmailPreview({
  ...
  const renderFormattedLetter = () => {
    ...
    lines.forEach((line, i) => {
      ...
      // Lines with bold text (wrapped in *)
  ...
  // Highlight dynamic values in the letter (amounts, dates)
  const highlightAmounts = (text) => {
    if (typeof text !== "string") return text;
    // Highlight euro amounts
    return text.replace(/(€[\s]?\d+[.,]\d{2})/g, '<span class="bg-yellow-100 dark:bg-yellow-900/30 px-1 rounded font-semibold text-primary">$1</span>');
  };
        ...
        <div className="flex-1">
          <div className="mb-4">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
              Controleer de inhoud voordat u het verzoek verstuurt.
            </p>
          </div>
                  ...
                  <div className="mt-2 p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800">
                    <div className="flex items-center gap-2">
                      <span className="text-emerald-600 dark:text-emerald-400 font-bold">
                        💡 Aanbevolen: €{calculation.voorstel.toFixed(2)}/maand
                      </span>
        ...
        <div className="w-full lg:w-64 flex flex-col gap-3">
          <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-slate-200 dark:border-[#2A2A2A] p-4 space-y-3 shadow-sm">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-500/10 text-blue-400 text-[9px] font-bold border border-blue-500/20">
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-[#2A2A2A]">
              <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-normal">
                Door op verzenden te klikken, wordt de status bijgewerkt. U kunt
                de tekst kopiëren of downloaden en zelf versturen per e-mail of
                post.
              </p>
            </div>
          </div>
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800 p-4">
          ...
          <ul className="space-y-2 text-xs text-emerald-800 dark:text-emerald-200">
            <li className="flex items-start gap-2">
              ...
              <span>
                Maak een overzicht van uw inkomsten en uitgaven waaruit blijkt
                wat u kunt betalen
              </span>
            </li>
            <li className="flex items-start gap-2">
              ...
              <span>
                Stuur onze voorbeeldbrief met het overzicht naar de schuldeiser
                of het incassobureau
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-emerald-600 dark:text-emerald-400 font-bold flex-shrink-0">
              ...
              <span>
                U kunt alvast een eerste betaling doen om te laten zien dat u
                zich aan de regeling wilt houden
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-emerald-600 dark:text-emerald-400 font-bold flex-shrink-0">
              ...
              <span>
                Vraag altijd om een schriftelijke bevestiging van de afspraken
              </span>
            </li>
          </ul>
        </div>
